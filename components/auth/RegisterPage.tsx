...
const RegisterPage: React.FC<RegisterPageProps> = ({ setActivePage }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await auth.register(name, username, email);
      // On successful registration, automatically log in and go to showcase
      ...
      <div className="w-full max-w-md bg-white dark:bg-gray-800 p-8 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={isLoading} className="w-full bg-blue-500 text-white font-bold py-3 px-4 rounded-lg hover:bg-blue-600 transition duration-300 disabled:bg-blue-300 flex justify-center items-center">
            {isLoading ? <div className="animate-spin rounded-full h-6 w-6 border-b-2 border-white"></div> : 'Create Account'}
          </button>
        </form>
        <p className="text-center text-sm text-gray-600 dark:text-gray-400 mt-6">
          Already have an account?{' '}
          <button onClick={() => setActivePage(Page.Login)} className="font-medium text-blue-600 dark:text-blue-400 hover:underline">
            Login
          </button>
        </p>
      </div>
